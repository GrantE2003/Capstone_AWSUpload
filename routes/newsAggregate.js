...
router.get('/aggregate', async (req, res) => {
  try {
    ...
    // For search queries, use Mediastack instead of Currents (as per user requirements)
    // For category browsing, continue using Currents
    const useMediastack = isSearch;
    ...
    // Use Mediastack for search queries, Currents for category browsing
    const thirdSourcePromise = useMediastack
    ...
    const [guardianResults, gdeltResults, thirdSourceResults] = await Promise.allSettled([
    ...
    const thirdSourceArticles =
      thirdSourceResults.status === 'fulfilled' ? thirdSourceResults.value : [];
    ...
    if (!Array.isArray(thirdSourceArticles)) {
      console.warn(`[Aggregate] ${useMediastack ? 'Mediastack' : 'Currents'} returned non-array:`, typeof thirdSourceArticles);
    }
    ...
    const thirdSourceCount = Array.isArray(thirdSourceArticles) ? thirdSourceArticles.length : 0;
    ...
    console.log(
      `   ${useMediastack ? 'Mediastack' : 'Currents'}: ${thirdSourceCount} articles ${thirdSourceCount === 0 ? '(NONE!)' : ''}`
    );

    const totalArticles = guardianCount + gdeltCount + thirdSourceCount;
    ...
    if (thirdSourceCount === 0) {
      console.warn(`[Aggregate] WARNING: ${useMediastack ? 'Mediastack' : 'Currents'} returned 0 articles - check API key and query`);
    }
    ...
    const allArticles = [
      ...
      ...(Array.isArray(thirdSourceArticles) ? thirdSourceArticles : [])
    ];
    ...
    const articlesWithSource = allArticles.map(article => ({
      ...
      source:
        article.sourceName === 'Guardian'
          ...
          ? 'gdelt'
          : article.sourceName === 'Currents' || article.sourceName === 'Mediastack'
          ? (article.sourceName === 'Mediastack' ? 'mediastack' : 'currents')
          : 'unknown'
    }));
    ...
    const thirdSourceCountCombined = articlesWithSource.filter(a => 
      a.source === (useMediastack ? 'mediastack' : 'currents')
    ).length;
    console.log(`   Guardian: ${guardianCountCombined} articles`);
    console.log(`   GDELT: ${gdeltCountCombined} articles`);
    console.log(`   ${useMediastack ? 'Mediastack' : 'Currents'}: ${thirdSourceCountCombined} articles`);
    console.log(`   Total: ${articlesWithSource.length} articles\n`);

    const sourceBreakdown = {
      guardian: guardianCountCombined,
      gdelt: gdeltCountCombined,
      [useMediastack ? 'mediastack' : 'currents']: thirdSourceCountCombined
    };
    console.log('[Aggregate] Source verification:', sourceBreakdown);

    const nonGuardianCount = gdeltCountCombined + thirdSourceCountCombined;
    if (nonGuardianCount === 0 && articlesWithSource.length > 0) {
      console.warn(
        `[Aggregate] WARNING: Only Guardian articles found. GDELT and ${useMediastack ? 'Mediastack' : 'Currents'} may not be working.`
      );
    } else {
      console.log(
        `[Aggregate] Non-Guardian articles: ${nonGuardianCount} (GDELT: ${gdeltCountCombined}, ${useMediastack ? 'Mediastack' : 'Currents'}: ${thirdSourceCountCombined})`
      );
    }
