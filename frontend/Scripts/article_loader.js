(function () {
  ...
  const API_BASE = (function () {
    ...
    // This works for all pages: root (/), subpages (/Pages/business.html), etc.
    ...
    if (isProductionDomain) {
      ...
      // This works for: /, /Pages/business.html, /Pages/technology.html, etc.
      const apiBase = window.location.origin; // e.g., https://www.4970capstone-mss.com
      console.log("[Article Loader] Production domain detected:", window.location.hostname);
      console.log("[Article Loader] Current page:", window.location.pathname);
      console.log("[Article Loader] Using same origin for API calls:", apiBase);
    ...
    if (window.API_BASE_URL) {
      console.log("[Article Loader] Using injected API_BASE_URL:", window.API_BASE_URL);
      return window.API_BASE_URL;
    }

    // Last resort: Try Render backend
    const renderUrl = "https://capstone-awsupload.onrender.com";
    console.log("[Article Loader] Using Render backend fallback:", renderUrl);
    return renderUrl;
  })();
  ...
  function renderStoryGroup(group, container) {
    ...
    summaryButton.addEventListener("click", async () => {
      ...
      try {
        ...
        // Extract text for summary from multiple possible sources
        ...
        // Validate that we have enough text to summarize
        if (!textForSummary || textForSummary.trim().length < 50) {
          throw new Error("Article text is too short to generate a summary. Please read the full article.");
        }
        ...
        if (!resp.ok) {
          ...
          let errorMessage = `Summary request failed: ${resp.status} ${resp.statusText}`;
          try {
        ...
        if (!result || !result.summary) {
          throw new Error("Invalid response from server: no summary provided");
        }
        ...
        console.log("[Article Loader] Summary loaded successfully");
      } catch (err) {
        console.error("[Article Loader] Error fetching summary:", err);
        // Provide user-friendly error message
        let errorMessage = "Sorry, we couldn't load a summary right now. Please try again later.";
        if (err.message) {
          errorMessage = err.message;
        }
        summaryText.textContent = errorMessage;
        // Don't mark as loaded so user can try again
