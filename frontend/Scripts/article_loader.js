(function () {
  ...
  function renderStoryGroup(group, container) {
    ...
    // Combined summary (matches screenshot style)
  ...
  function loadArticles() {
    ...
    const apiUrl = buildApiUrl(category);
    console.log('[Article Loader] Making fetch request to:', apiUrl);
    
    fetch(apiUrl)
      .then(response => {
        console.log('[Article Loader] Response status:', response.status, response.statusText);
        console.log('[Article Loader] Response URL:', response.url);
        if (!response.ok) {
      ...
      .then(data => {
        ...
        if (data.groupedArticles && data.groupedArticles.length > 0) {
          ...
          console.log('[Article Loader] Displaying', data.groupedArticles.length, 'grouped stories');
          ...
          articlesContainer.innerHTML = '<div class="card"><p>No grouped stories available for this topic. Articles may be too dissimilar to group together.</p></div>';
        }
      ...
      .catch(error => {
        ...
        console.error('[Article Loader] Full URL attempted:', apiUrl);
